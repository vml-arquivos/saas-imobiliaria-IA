...
// ============================================
// GESTÃO DE ALUGUÉIS - PROPRIETÁRIOS
// ============================================
...
export const propertyOwners = mysqlTable("property_owners", {
  ...
  documentNumber: varchar("documentNumber", { length: 20 }).notNull().unique(),
  email: varchar("email", { length: 320 }),
  phone: varchar("phone", { length: 20 }),
  whatsapp: varchar("whatsapp", { length: 20 }),
...
export const tenants = mysqlTable("tenants", {
  ...
  // Dados pessoais
  name: varchar("name", { length: 255 }).notNull(),
  documentNumber: varchar("documentNumber", { length: 20 }).notNull(),
  email: varchar("email", { length: 320 }),
  phone: varchar("phone", { length: 20 }),
  whatsapp: varchar("whatsapp", { length: 20 }),
  ...
  monthlyIncome: int("monthlyIncome"), // em centavos
  companyName: varchar("companyName", { length: 255 }),
  
  // Referências
  references: text("references"), // JSON com referências pessoais
...
export const rentalContracts = mysqlTable("rental_contracts", {
  ...
  // Valores (em centavos)
  rentAmount: int("rentAmount").notNull(), // Valor do aluguel
  condoFee: int("condoFee").default(0), // Condomínio
  iptu: int("iptu").default(0), // IPTU
  
  // Repasse e comissão
  ownerPercentage: int("ownerPercentage").default(90).notNull(), // % para proprietário
  commissionPercentage: int("commissionPercentage").default(10).notNull(), // % comissão
  
  // Vencimento
  dueDay: int("dueDay").notNull(), // Dia do vencimento (1-31)
  
  // Garantias
  depositAmount: int("depositAmount").default(0), // Caução
  guaranteeType: mysqlEnum("guaranteeType", ["caucao", "fiador", "seguro", "nenhum"]),
  
  // Status
  status: mysqlEnum("status", ["ativo", "encerrado", "inadimplente", "em_renovacao"]).default("ativo").notNull(),
...
export const financialTransactions = mysqlTable("financial_transactions", {
  ...
  // Método de pagamento
  paymentMethod: mysqlEnum("paymentMethod", ["pix", "transferencia", "boleto", "dinheiro", "cartao"]),
...
// ============================================
// GESTÃO FINANCEIRA - DESPESAS DE IMÓVEIS
// ============================================
...
export const propertyExpenses = mysqlTable("property_expenses", {
  ...
  // Relacionamentos
  propertyId: int("propertyId").notNull(),
  contractId: int("contractId"), // Opcional, pode ser despesa sem contrato
  ...
  paidBy: mysqlEnum("paidBy", ["proprietario", "inquilino", "imobiliaria"]).notNull(),
...
// ============================================
// TABELA DE PAPÉIS/PERMISSÕES DE USUÁRIOS
// ============================================
...
export const userRoles = mysqlTable("user_roles", {
  ...
  permissions: text("permissions"), // ["properties.create", "leads.view", "reports.export"]
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  assignedBy: int("assignedBy"), // Quem atribuiu o papel
});
...
export const visits = mysqlTable("visits", {
  ...
  leadId: int("leadId").notNull(),
  propertyId: int("propertyId").notNull(),
  assignedTo: int("assignedTo"), // Corretor responsável
  ...
  clientInterest: mysqlEnum("clientInterest", [
    "muito_interessado",
    "interessado",
    "neutro",
    "pouco_interessado",
...
export const proposals = mysqlTable("proposals", {
  ...
  leadId: int("leadId").notNull(),
  propertyId: int("propertyId").notNull(),
  createdBy: int("createdBy").notNull(), // Corretor que criou
  
  // Valores propostos
  proposedPrice: int("proposedPrice").notNull(), // em centavos
  ...
  downPayment: int("downPayment"), // entrada em centavos
  installments: int("installments"), // número de parcelas
  installmentValue: int("installmentValue"), // valor da parcela em centavos
  
  // Detalhes
  description: text("description"),
  terms: text("terms"), // Condições e termos
  notes: text("notes"),
...
export const documents = mysqlTable("documents", {
  ...
  // Relacionamentos (pode ser vinculado a diferentes entidades)
...
export const tasks = mysqlTable("tasks", {
  ...
  leadId: int("leadId"),
  propertyId: int("propertyId"),
  assignedTo: int("assignedTo").notNull(), // Responsável
...
export const activityLogs = mysqlTable("activity_logs", {
  ...
  // Detalhes
  description: text("description"),
  changes: text("changes"), // JSON com before/after
...
// ============================================
// TABELA DE ESTÁGIOS DO PIPELINE/FUNIL
// ============================================
...
export const pipelineStages = mysqlTable("pipeline_stages", {
  ...
  pipelineType: mysqlEnum("pipelineType", [
    ...
    "locacao",
    "captacao"
  ]).default("vendas").notNull(),
  
  // Configurações
  isActive: boolean("isActive").default(true).notNull(),
  isClosedStage: boolean("isClosedStage").default(false), // Estágio final (ganho/perdido)
  isWonStage: boolean("isWonStage").default(false), // Estágio de vitória
  
  // Automações (JSON)
  automations: text("automations"), // Ações automáticas ao entrar no estágio
