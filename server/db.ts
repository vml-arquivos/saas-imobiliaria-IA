...
export async function getAllPropertyOwners() {
  ...
  if (!database) return [];
  const { propertyOwners } = await import("../drizzle/schema");
  return await database.select().from(propertyOwners).where(eq(propertyOwners.active, true));
}
...
export async function getPropertyOwnerById(id: number) {
  ...
  const { propertyOwners } = await import("../drizzle/schema");
  const result = await database.select().from(propertyOwners).where(eq(propertyOwners.id, id)).limit(1);
  return result[0] || null;
}

export async function createPropertyOwner(data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { propertyOwners } = await import("../drizzle/schema");
  const result = await database.insert(propertyOwners).values(data);
  return { id: result[0].insertId, ...data };
}

export async function updatePropertyOwner(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { propertyOwners } = await import("../drizzle/schema");
  await database.update(propertyOwners).set(data).where(eq(propertyOwners.id, id));
  return { id, ...data };
}

export async function deletePropertyOwner(id: number) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { propertyOwners } = await import("../drizzle/schema");
  await database.update(propertyOwners).set({ active: false }).where(eq(propertyOwners.id, id));
  return { success: true };
}
...
export async function getAllTenants() {
  ...
  return await database.select().from(tenants).where(eq(tenants.active, true));
}
...
export async function createTenant(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { tenants } = await import("../drizzle/schema");
  const result = await database.insert(tenants).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function updateTenant(id: number, data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { tenants } = await import("../drizzle/schema");
  await database.update(tenants).set(data).where(eq(tenants.id, id));
  return { id, ...data };
}
...
export async function deleteTenant(id: number) {
  ...
  if (!database) throw new Error("Database not available");
  const { tenants } = await import("../drizzle/schema");
  await database.update(tenants).set({ active: false }).where(eq(tenants.id, id));
  return { success: true };
}
...
export async function getAllRentalContracts(status?: string) {
  const database = await getDb();
  if (!database) return [];
  const { rentalContracts } = await import("../drizzle/schema");
  if (status) {
    return await database.select().from(rentalContracts).where(eq(rentalContracts.status, status as any));
  }
  return await database.select().from(rentalContracts);
}
...
export async function getRentalContractById(id: number) {
  ...
  if (!database) return null;
  const { rentalContracts } = await import("../drizzle/schema");
  const result = await database.select().from(rentalContracts).where(eq(rentalContracts.id, id)).limit(1);
  return result[0] || null;
}

export async function createRentalContract(data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { rentalContracts } = await import("../drizzle/schema");
  const result = await database.insert(rentalContracts).values(data);
  return { id: result[0].insertId, ...data };
}

export async function updateRentalContract(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { rentalContracts } = await import("../drizzle/schema");
  await database.update(rentalContracts).set(data).where(eq(rentalContracts.id, id));
  return { id, ...data };
}
...
export async function deleteRentalContract(id: number) {
  ...
  if (!database) throw new Error("Database not available");
  const { rentalContracts } = await import("../drizzle/schema");
  await database.delete(rentalContracts).where(eq(rentalContracts.id, id));
  return { success: true };
}
...
export async function getAllFinancialTransactions(filters?: any) {
  const database = await getDb();
  if (!database) return [];
  const { financialTransactions } = await import("../drizzle/schema");
  return await database.select().from(financialTransactions);
}
...
export async function createFinancialTransaction(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { financialTransactions } = await import("../drizzle/schema");
  const result = await database.insert(financialTransactions).values(data);
  return { id: result[0].insertId, ...data };
}

export async function registerPayment(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { financialTransactions } = await import("../drizzle/schema");
  await database.update(financialTransactions).set({
    status: "pago",
...
export async function getFinancialDashboard(month: string) {
  const database = await getDb();
  if (!database) return null;
  const { financialTransactions } = await import("../drizzle/schema");
  
  const transactions = await database.select().from(financialTransactions)
    .where(eq(financialTransactions.referenceMonth, month));
  
  const totalReceived = transactions
    .filter(t => t.type === "recebimento_aluguel" && t.status === "pago")
    .reduce((sum, t) => sum + (t.amount || 0), 0);
  
  const totalCommissions = transactions
    .filter(t => t.type === "comissao_imobiliaria" && t.status === "pago")
    .reduce((sum, t) => sum + (t.amount || 0), 0);
...
export async function getAllPropertyExpenses(filters?: any) {
  const database = await getDb();
  if (!database) return [];
  const { propertyExpenses } = await import("../drizzle/schema");
  return await database.select().from(propertyExpenses);
}

export async function createPropertyExpense(data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { propertyExpenses } = await import("../drizzle/schema");
  const result = await database.insert(propertyExpenses).values(data);
  return { id: result[0].insertId, ...data };
}

export async function updatePropertyExpense(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { propertyExpenses } = await import("../drizzle/schema");
  await database.update(propertyExpenses).set(data).where(eq(propertyExpenses.id, id));
  return { id, ...data };
}
...
export async function getAllVisits() {
  ...
  return await database.select().from(visits);
}
...
export async function getVisitById(id: number) {
  ...
  if (!database) return null;
  const { visits } = await import("../drizzle/schema");
  const result = await database.select().from(visits).where(eq(visits.id, id));
  return result[0] || null;
}
...
export async function createVisit(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { visits } = await import("../drizzle/schema");
  const result = await database.insert(visits).values(data);
  return { id: result[0].insertId, ...data };
}

export async function updateVisit(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { visits } = await import("../drizzle/schema");
  await database.update(visits).set(data).where(eq(visits.id, id));
  return { id, ...data };
}
...
export async function getAllProposals() {
  ...
  return await database.select().from(proposals);
}
...
export async function getProposalById(id: number) {
  ...
  if (!database) return null;
  const { proposals } = await import("../drizzle/schema");
  const result = await database.select().from(proposals).where(eq(proposals.id, id));
  return result[0] || null;
}
...
export async function createProposal(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { proposals } = await import("../drizzle/schema");
  const result = await database.insert(proposals).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function updateProposal(id: number, data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { proposals } = await import("../drizzle/schema");
  await database.update(proposals).set(data).where(eq(proposals.id, id));
  return { id, ...data };
}
...
export async function getDocumentById(id: number) {
  ...
  if (!database) return null;
  const { documents } = await import("../drizzle/schema");
  const result = await database.select().from(documents).where(eq(documents.id, id));
  return result[0] || null;
}
...
export async function createDocument(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { documents } = await import("../drizzle/schema");
  const result = await database.insert(documents).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function createTask(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { tasks } = await import("../drizzle/schema");
  const result = await database.insert(tasks).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function updateTask(id: number, data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { tasks } = await import("../drizzle/schema");
  await database.update(tasks).set(data).where(eq(tasks.id, id));
  return { id, ...data };
}
...
export async function createActivityLog(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { activityLogs } = await import("../drizzle/schema");
  const result = await database.insert(activityLogs).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function getAllPipelineStages() {
  const database = await getDb();
  if (!database) return [];
  const { pipelineStages } = await import("../drizzle/schema");
  return await database.select().from(pipelineStages);
}
...
export async function getPipelineStageById(id: number) {
  ...
  if (!database) return null;
  const { pipelineStages } = await import("../drizzle/schema");
  const result = await database.select().from(pipelineStages).where(eq(pipelineStages.id, id));
  return result[0] || null;
}
...
export async function createPipelineStage(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { pipelineStages } = await import("../drizzle/schema");
  const result = await database.insert(pipelineStages).values(data);
  return { id: result[0].insertId, ...data };
}
...
export async function updatePipelineStage(id: number, data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { pipelineStages } = await import("../drizzle/schema");
  await database.update(pipelineStages).set(data).where(eq(pipelineStages.id, id));
  return { id, ...data };
}
...
export async function createUserRole(data: any) {
  ...
  if (!database) throw new Error("Database not available");
  const { userRoles } = await import("../drizzle/schema");
  const result = await database.insert(userRoles).values(data);
  return { id: result[0].insertId, ...data };
}

export async function updateUserRole(id: number, data: any) {
  const database = await getDb();
  if (!database) throw new Error("Database not available");
  const { userRoles } = await import("../drizzle/schema");
  await database.update(userRoles).set(data).where(eq(userRoles.id, id));
  return { id, ...data };
}
